...
Compare the above response curve with the FOP plot to see how the model captures the observed pattern:
...
**Exercise:** Compare the FOP plots with the response curves for all variables in your final model. Where do you see good agreement? Where does the model seem to smooth over or (conversely) overfit occurrence patterns? Notice that the scale of the Y-axis in the response curves is unbounded [0,inf] --- with presence-only data, the model can only estimate relative suitability, not absolute probability of occurrence.

```{r fop-resp, include=FALSE}
evs <- (train_model$selectedmodel$coefficients)[-1] |>
  names() |>
  sub("_.*$", "", x = _) |>
  unique()
for (ev in evs) {
  plotFOP(train_data, ev)
  plotResp(
    train_model$selectedmodel,
    train_DVs$transformations,
    ev
