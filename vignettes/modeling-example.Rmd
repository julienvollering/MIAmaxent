...
>__Variable selection occurs heirarchically: first derived variables within each explanatory variable, then explanatory variables within the full model.__
...
```
We can see that `selectEV` has reduced the number of explanatory variables, but it has also added new interaction terms between explanatory variables selected for the model.
...
For example, we may decide that a simple model with only 5 explanatory varibles is best, since rounds 6-13 explain only small amounts of additional variation. Thus, we choose the best model in round 5:
```{r}
...
```{r, fig.width=5, fig.height=5}
pr_bygallResp2 <- plotResp2(grasslandPO, "pr_bygall", transformation = system.file("extdata", "transformations.Rdata", package = "maxentmodelselectr"), model = system.file("extdata", "1.lambdas", package = "maxentmodelselectr"))
```
...
```{r}
grasslandPrediction <- projectModel(grasslandPO, transformation = system.file("extdata", "transformations.Rdata", package = "maxentmodelselectr"), model = system.file("extdata", "1.lambdas", package = "maxentmodelselectr"))
```
...
```{r}
head(grasslandPrediction$ranges)
```
Since we projected the model over the training data, it makes sense that all the ranges are reported as [0,1] or "inside".

Here is some code (using the `raster` package again) that shows how to plot model predictions spatially:
```{r, fig.show='hold', fig.width=5, fig.height=5, warning=FALSE, message =FALSE, results='hide', fig.cap="Model predictions in probability ratio output (PRO) across the study area"}
library(raster)
...
spatialPrediction <- projectModel(stackpts, transformation = system.file("extdata", "transformations.Rdata", package = "maxentmodelselectr"), model = system.file("extdata", "1.lambdas", package = "maxentmodelselectr"))
