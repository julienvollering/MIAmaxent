...
>__All functions in `MIAmaxent` return console output. Therefore it's handy to always assign function output to an object, so that you can manipulate that object further.__  
...
```
We can see that `selectEV` has reduced the number of explanatory variables to 8, but it has also added 3 new interaction terms between explanatory variables selected for the model.
...
>__The trail of forward variable selection is automatically written to file (in the specified `dir`), and can be examined more closely using spreadsheet software.__
...
For example, we may decide that a simple model with only 4 explanatory variables is best, since rounds 5-11 explain only small amounts of additional variation. Thus, we choose the best model in round 4:
```{r}
grasslandEVselect[[2]][grasslandEVselect$selection$round == 4, ][1, -13]
```
The explanatory variables included in this model are: pr-bygall, terslpps15, lcucor1, and geoberg.
...
grasslandPOXY <- readData(occurrence = system.file("extdata", "occurrence_PO.csv", package = "MIAmaxent"), 
    contEV = system.file("extdata", "EV_continuous", package = "MIAmaxent"),
    catEV = system.file("extdata", "EV_categorical", package = "MIAmaxent"),
    maxbkg = 20000, XY = TRUE)
spatialPrediction <- projectModel(grasslandPOXY, transformation = grasslandDVs[[2]], 
    model = system.file("extdata", "1.lambdas", package = "MIAmaxent"))

library(raster)
Predictionraster <- raster(list.files(system.file("extdata", "EV_continuous", package = "MIAmaxent"), full.names = TRUE)[1])
