...
plotResp3 <- function(data, transformations, model, EV, logscale = FALSE, ...) {
  ...
  margind <- grepl(paste0(evname, "_"), names(model$betas))
  EVind <- gsub("_.*", "", names(model$betas))
  ...
  newdata <- mapply(function(f, margind, numericind) {
    if (margind) {
      f(seq)
    } else {
  ...
  margdata <- lapply(modtransfs[margind], function(f, x) { f(x) }, x=data[,evname])
  consdata <- lapply(modtransfs[consind], function(f, x) { f(x) }, x=data[,evname])
